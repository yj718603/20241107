      ...
      <div style="display:flex;z-index:1">
        <div style="flex:1">
          <div style="height:40px;line-height:40px;display:flex">
            ...
            <div class="translator-icon-arrow" @click.stop="isArrow1=!isArrow1">
              <span class="iconfont icon-xiala" :class="{'icon-xiala-rotate':isArrow1}"/>
            </div>
          </div>
          <div style="margin-top:6px;position:relative">
            <editorInput @input="checkLanguage" v-model="sourceText"></editorInput>
        ...
        <div style="flex:1;margin-left:10px;display:flex;flex-direction:column;position:relative">
          ...
          <div style="height:40px;line-height:40px;padding-left:15px;display:flex">
            ...
            <div class="translator-icon-arrow" @click.stop="isArrow2=!isArrow2">
          ...
          <div style="margin-top:6px;flex:1;background:#F5F5FA;padding:10px">
            <span class="target-item" :key="index" v-for="(item,index) in targetText" @mouseover="addHover(index)"
        ...
        <div v-if="checkedArrayTranslate.length>1" :class="{'animate__fadeInRight':checkedArrayTranslate.length>1}"
          ...
          <div style="margin-top:46px;flex:1;background:#F5F5FA;padding:10px">
            <span :key="index" class="target-item" v-for="(item,index) in targetText2" @mouseover="addHover(index)"
                  @mouseout="removeHover(index)">
              {{item.t}}
            </span>
          </div>
      ...
      <div class="original-languages-popover" v-show="(isArrow1 || isArrow2)">
        ...
        <div class="original-languages">
          <template v-for="(item,index) in configLanguages">
            <div :key="item.code" class="item-language" v-show="isArrow1 ||(isArrow2 && index!=0)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addHover(index) {
        $("#translator-app-editor-input").find("per").eq(index).addClass("per-active");
      },
      removeHover(index) {
        $("#translator-app-editor-input").find("per").eq(index).removeClass("per-active");
      },
      checkLanguage(value) {
      ...
      translateSource() {
        ...
        this.$axios.post("v1/translateText:web", request)
          .then((res) => {
            this.targetText = res.r[0].translation;
            this.targetText2 = res.r[1].translation;
            //
            /* const arrSourceText=[];
             this.targetText.forEach((item)=>{
               arrSourceText.push("<per>"+item.s+"</per>");
             });
             this.sourceText=arrSourceText.join();*/
          });
      },
      clearSource() {
        this.sourceText = "";
